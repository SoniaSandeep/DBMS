...
select e.empno from emp e where e.empno not in(select empno from incentives);
...
SELECT * FROM emp e
WHERE e.empno IN
(SELECT mgr_no FROM emp where sal> (SELECT avg(m.sal)FROM m.emp WHERE m.mgr_no = e.empno));
